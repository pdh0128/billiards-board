...
- Components are PascalCase; hooks are prefixed with `use`; route handlers mirror API paths (e.g., `app/api/article/route.ts`).
- Prefer descriptive kebab-case filenames for multiword components (e.g., `article-form.tsx`) and keep UI logic lightweight with helpers in `utils/`.
- Tailwind utilities are the default styling approach; avoid inline styles unless necessary.

## Testing Guidelines
- No automated suite yet; at minimum run `npm run lint` and validate flows manually in `npm run dev` (article creation, collisions/comments, deletions, socket sync).
